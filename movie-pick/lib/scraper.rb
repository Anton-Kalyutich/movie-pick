class Scraper
  ...
  def self.scrape_suggestmemovie
    ...
    movie_info[:plot] = info.css("p.card-text").text.strip
    movie_info[:genre] = info.css("dl.row dd.col-sm-9:first-of-type").text
    movie_info[:language] = info.css("dl.row dd.col-sm-9:nth-of-type(2)")
